<p align="center">
  <img src="慕课栈-扩展.assets/logo.png" alt="智慧MOOC教育平台 Logo" width="300" />
</p>
...
| 模块名称     | 模块定位   | 模块介绍                                                     |
| ------------ | ---------- | ------------------------------------------------------------ |
| mkz-api       | 接口服务   | 提供统一的API服务，方便内部系统跨服务调用功能                |
| mkz-auth      | 鉴权中心   | 负责平台的认证和授权相关功能，处理用户登录、权限验证等操作   |
| mkz-chat      | AI服务中心 | 实现平台内AI智能对话、知识库管理、AI工具服务调用等操作       |
| mkz-common    | 通用包     | 存放项目通用的代码、工具类、常量等，供其他模块复用           |
| mkz-course    | 课程中心   | 管理课程相关业务，如课程的创建、编辑、展示、查询等           |
| mkz-data      | 数据中心   | 涉及管理端数据展板展示、基于网关日志进行数据分析、流量统计、报表生成等功能 |
| mkz-exam      | 考试中心   | 用于考试相关功能，包括考试管理、成绩记录等                   |
| mkz-gateway   | 网关       | 作为网关，处理请求的路由、过滤、鉴权等，保障系统的安全和流量管理 |
| mkz-learning  | 学习中心   | 专注于学习相关业务，也包含用户学习的各种辅助功能             |
| mkz-live      | 直播中心   | 支持直播功能，如直播课程的创建、直播流管理、观众互动等       |
| mkz-media     | 媒资管理   | 用于管理媒体资源，如媒资、文件的管理存储                     |
| mkz-message   | 消息中心   | 负责平台系统消息/通知推送、以及用户私聊、在线群聊的功能      |
| mkz-pay       | 支付中心   | 集成了多种支付方式，处理支付相关业务，如第三方支付或退款、支付方式管理、支付状态查询、对账等 |
| mkz-promotion | 营销中心   | 管理平台的促销活动，如优惠券发放、折扣活动设置等             |
| mkz-remark    | 互动中心   | 用于处理评论、评价等相关功能，对点赞等操作进行专门统计存储   |
| mkz-search    | 搜索服务   | 提供搜索功能，支持用户对课程、资料等内容的搜索及提供个性化推荐 |
| mkz-trade     | 交易中心   | 处理交易相关业务，如订单管理、交易记录查询等                 |
| mkz-user      | 用户中心   | 管理用户相关业务，如用户信息的增删改查、用户角色管理等       |


## 💻 前端模块介绍

注：前端模块在mkz-front文件夹下

| 模块名称   |  模块定位 | 模块介绍                    |
|-----------|-------|-------------------------|
| mkz-admin  | 管理端  | 提供后台管理功能。只有后台用户、教师可以登录。 |
| mkz-protal | 前台    | 围绕课程提供服务。只有学生端用户可以登录。   |
...
> 相关中间件的详细安装与部署说明可参考[`慕课栈-扩展.md`](./慕课栈-扩展.md)
...
- [`慕课栈-扩展.md`](./慕课栈-扩展.md)：项目改造笔记（基于原始「慕课栈」项目的改造说明）
...
项目基于慕课栈进行改造，改造笔记在 [项目改造笔记](慕课栈-扩展.md) 中，每一步改造都包含当时的技术考量与取舍，主要用于系统性学习与实践总结。
...
![公众号](慕课栈-扩展.assets/%E5%85%AC%E4%BC%97%E5%8F%B7.jpg)
