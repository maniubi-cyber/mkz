...
> JDK17分支新功能：
> 
> 1. **AIGC模块**（基于SpringAI）：实现智能问答、课程推荐、语音转文字、AI朗读、文生图等功能
> 2. **直播模块**（基于Nginx+本地OBS推流）：实现视频点播、视频推流
> 3. **IM模块**（基于Websocket）：实现直播间聊天、刷礼物、直播间推送公告等功能
> 4. **搜索模块**（基于elasticsearch8.x版本）：实现高版本elasticsearch的搜索功能（抛弃RestHighLevelClient）
> 
