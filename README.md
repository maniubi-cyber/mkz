...
| Milvus | 2.4+ | 向量数据库（父切块存储，替代 Chroma） |
...
| 切块类型 | 大小 | 存储 | 用途 |
|----------|------|------|------|
| 父切块 | 1000字符 | Milvus | 向量检索召回，保持完整段落结构 |
| 子切块 | 200字符 | Redis | 精确匹配和 LLM 输入，减少 token 消耗 |

**Milvus vs Chroma 优势：**
- 支持亿级向量数据（Chroma 适合中小规模）
...
| Attu (Milvus) | http://localhost:8001 | Milvus 可视化管理 |
