...
@documents_router.post(
    ...
    **完整处理流水线：**
    ...
    3. **文本切分** (TextChunker)
    4. **向量化 + 写入 Milvus** (BGE-large-zh → kb_{id})
...
async def parse_document(request: ParseRequest):
    ...
    parse → chunk → embed → Milvus → BM25 rebuild
    """
...
@documents_router.post(
    ...
    response_model=dict,
    summary="删除文档向量",
    description="删除指定文档在 Milvus 向量库中的所有向量数据。",
    responses={
...
@ai_router.post(
    ...
    **混合检索流水线：**
    ...
    1. **向量语义检索** — Embed query → Milvus similarity search
