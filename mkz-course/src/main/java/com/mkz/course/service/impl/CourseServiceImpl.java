...
import com.mkz.api.client.exam.ExamClient;
...
import com.mkz.api.dto.IdAndNumDTO;
import com.mkz.api.dto.course.*;
import com.mkz.api.dto.leanring.LearningLessonDTO;
import com.mkz.api.dto.leanring.LearningRecordDTO;
import com.mkz.api.dto.user.UserDTO;
import com.mkz.common.autoconfigure.mq.RabbitMqHelper;
import com.mkz.common.constants.ErrorInfo;
...
import com.mkz.common.exceptions.DbException;
import com.mkz.common.utils.*;
import com.mkz.course.constants.CourseErrorInfo;
import com.mkz.course.constants.CourseStatus;
import com.mkz.course.constants.RedisContants;
import com.mkz.course.config.TwoLevelCacheManager;
import com.fasterxml.jackson.core.type.TypeReference;
import java.util.concurrent.TimeUnit;
import com.mkz.course.domain.dto.CoursePageQuery;
import com.mkz.course.domain.dto.CourseSimpleInfoListDTO;
import com.mkz.course.domain.po.Category;
import com.mkz.course.domain.po.Category3PO;
import com.mkz.course.domain.po.Course;
import com.mkz.course.domain.po.CourseTeacher;
import com.mkz.course.domain.vo.*;
import com.mkz.course.mapper.CourseDraftMapper;
import com.mkz.course.mapper.CourseMapper;
import com.mkz.course.mapper.CourseTeacherMapper;
import com.mkz.course.service.*;
...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements ICourseService {
    ...
    public List<CourseSimpleInfoDTO> getSimpleInfoList(CourseSimpleInfoListDTO courseSimpleInfoListDTO) {
        // 1.若传入课程id列表，则按id逐个走双层缓存，提升热点课程响应速度
        List<Long> ids = courseSimpleInfoListDTO.getIds();
        if (CollUtils.isNotEmpty(ids)) {
            List<CourseSimpleInfoDTO> result = new ArrayList<>();
            for (Long id : ids) {
                String localKey = "course:simple:" + id;
                CourseSimpleInfoDTO cached = twoLevelCacheManager.get(localKey, localKey, 30, TimeUnit.MINUTES,
                        () -> {
                            Course course = baseMapper.selectById(id);
                            return course == null ? null : BeanUtils.toBean(course, CourseSimpleInfoDTO.class);
                        },
                        new TypeReference<CourseSimpleInfoDTO>() {});
                if (cached != null) {
                    result.add(cached);
                }
            }
    ...
    public int courseFinished() {
        ...
        //6.失效课程缓存（状态变更，避免残留旧数据）
        for (Course course : courses) {
            evictCourseCache(course.getId());
        }
    ...
    public CourseFullInfoDTO getInfoById(Long id, boolean withCatalogue, boolean withTeachers) {
        // 走双层缓存，缓存key按参数区分，保证不同参数返回结果互不污染
        String localKey = "course:info:" + id + ":" + withCatalogue + ":" + withTeachers;
        return twoLevelCacheManager.get(localKey, localKey, 30, TimeUnit.MINUTES,
                () -> loadCourseFullInfoFromDb(id, withCatalogue, withTeachers),
                new TypeReference<CourseFullInfoDTO>() {});
    }

    /**
     * 从数据库加载课程完整信息（缓存miss时回源）
     */
    private CourseFullInfoDTO loadCourseFullInfoFromDb(Long id, boolean withCatalogue, boolean withTeachers) {
        // 1.查询课程基本信息
        Course course = baseMapper.selectById(id);
        if (course == null) {
    ...
    public void evictCourseCache(Long id) {
        // 清理课程详情缓存（4种参数组合）
        for (boolean wc : new boolean[]{true, false}) {
            for (boolean wt : new boolean[]{true, false}) {
                String k = "course:info:" + id + ":" + wc + ":" + wt;
                twoLevelCacheManager.evict(k, k);
            }
        }
        // 清理课程简单信息缓存
        String sk = "course:simple:" + id;
        twoLevelCacheManager.evict(sk, sk);
    }
