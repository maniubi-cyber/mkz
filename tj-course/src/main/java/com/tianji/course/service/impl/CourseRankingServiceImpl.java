...
import com.tianji.course.domain.vo.CourseRankingVO;
import com.tianji.course.mapper.CourseRankingMapper;
...
public class CourseRankingServiceImpl implements ICourseRankingService {
    ...
    public int archiveSeasonData(String seasonId) {
        ...
        // 批量写入MySQL
        List<CourseRanking> records = tuples.stream()
                .map(tuple -> {
        ...
        // 批量插入
        for (CourseRanking record : records) {
            courseRankingMapper.insert(record);
        }
